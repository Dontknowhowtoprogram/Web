    ...
    <script>   
        $(document).ready(function(){//定时
            setInterval("refresh()",1000);
               ...
               $.ajax({
                    type:'GET',
                    url:"xianshi?username=${sessionScope.username}",
                    async:false,
                    cache:false,
                    timeout:1000,
	...
	<body>
		...
		<table border="0" align="center" cellpadding="0" cellspacing="0" width="800">
			<tr background="img/bg1.jpg">
				...
				<a href="/zuorui/xiaoxi?username=${sessionScope.username}" id="count">消息(${requestScope.count})</a>&nbsp;&nbsp;&nbsp;
													...
													<c:forEach var="h" items="${requestScope.list}" varStatus="status">
												         ...
												         <li style="font-size:20px;">
												            ...
												            <span style="display:block;font-size:28px;color:#D52B6F;background:#f0f0f0;border:none solid;padding:10px 40px;width:700px;border-radius:25px;">
												              	...
												              	<input type="button" class="check" value="评论" style="text-align:centre; width:50px;border-radius:20px; padding:1px 10px;"/>
												              	<form action="/zuorui/message?titleid=${h.titleid}&username=${sessionScope.username}" method="post" onsubmit="return pinglun(this);">
												              	<textarea rows="4" cols="30" class="inputgri" name="text"  tabindex="0" style="display:none;"></textarea>
												              	<input type="submit" class="text" value="评论"  style="display:none;text-align:centre; width:50px;border-radius:20px; padding:1px 10px;"/>
												              	</form></font></div>
												        ...
												        <div style="WIDTH: 750; HEIGHT: 750; OVERFLOW: scroll; scrollbar-face-color:#70807d; scrollbar-arrow-color:#ffffff; scrollbar-highlight-color:#ffffff; scrollbar-3dlight-color:#70807d; scrollbar-shadow-color:#ffffff; scrollbar-darkshadow-color:#70807d; scrollbar-track-color:#ffffff">
														<c:forEach var="c" items="${requestScope.list1}">
														<c:if test="${c.titleid==h.titleid}"> 
														<div style="margin-left:20px; margin-top:15px; padding:5px 40px; border:none solid; border-radius:25px; width:fit-content; background:#FFFFFF;"><font size=3;><span style="color:#113DEE;text-align:left;">${c.username}:</span>&nbsp;${c.text}<br><c:forEach var="x" items="${requestScope.list3}">
														<c:if test="${x.textid==c.textid}"><span style="font-size:30%;color:red;text-align:right">(老师评分：${x.score})</span></c:if></c:forEach></div></font>
														<div align="left" style="margin-left:50px;  font-size:50%;">
														<c:if test="${sessionScope.username!=c.username}">
															<input type="button" class="check1" value="回复" style="text-align:centre; width:50px;border-radius:20px; padding:1px 10px;"/>
													        ...
													        <textarea rows="4" cols="30" class="inputgri1" name="intext"  tabindex="0" style="display:none;"></textarea>
													        <input type="submit" class="text1" value="评论" style="display:none;text-align:centre; width:50px;border-radius:20px; padding:1px 10px;"/>
													        </form>
													    </c:if>
														...
														<c:if test="${a.textid==c.textid}"><br><div id="${a.huifuid}" style=" margin-left:50px;text-align:left; padding:5px 40px; border:none solid; border-radius:25px; background:#FFFFFF;"><span style="color:#113DEE;">${a.name}&nbsp;<span style="font-size:30%">回复&nbsp;</span>${a.username}</span>:&nbsp;${a.intext}</div>
														...
														<font size=1>
															...
															<input type="button" class="check2" value="回复" style="text-align:centre; width:50px;border-radius:20px; padding:1px 10px;"/>
													        ...
													        <textarea rows="4" cols="30" class="inputgri2" name="intext"  tabindex="0" style="display:none;"></textarea>
													        <input type="submit" class="text2" value="评论" style="display:none;text-align:centre; width:50px;border-radius:20px; padding:1px 10px;"/>
													        </form>
															</c:if>
														</font>
														</c:if>
